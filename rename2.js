/**
 * 更新日期：2024-04-05 15:30:15（修改版：默认开启 flag 和 nf）
 * 用法：Sub-Store 脚本操作添加
 * 参数说明：详见原脚本注释
 *
 * 修改说明：
 * - flag 默认开启（原为关闭），如需关闭请在参数中写 flag=false
 * - nf   默认开启（原为关闭），如需关闭请在参数中写 nf=false
 * - 其他参数（bl, blgd, clear, nx 等）默认关闭，与原版一致
 */

// const inArg = {'blkey':'iplc+GPT>GPTnewName+NF+IPLC', 'flag':true };
const inArg = $arguments; // console.log(inArg)

// 修改点：nf 和 addflag 默认设为 true（如果未定义）
const nx = inArg.nx || false,
  bl = inArg.bl || false,
  nf = inArg.nf === undefined ? true : inArg.nf,      // 默认 true
  key = inArg.key || false,
  ...
  addflag = inArg.flag === undefined ? true : inArg.flag,  // 默认 true
  nm = inArg.nm || false;
